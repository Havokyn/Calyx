...
- **Repo sync is part of state management:** before starting a slice, before closing an issue, and after any long-running FSV, run `gh pr list` / `gh issue list` plus `git fetch` on aiwonder for both `/home/croyse/calyx/repo` (`ChrisRoyse/Calyx-Dev`) and `/home/croyse/calyx/public-mirror` (`ChrisRoyse/Calyx`). Merge or account for other agents' remote pushes/PRs, commit and push every completed change to dev, mirror public-safe changes to `Calyx`, then fast-forward the Windows checkouts without overwriting unrelated dirty files. Record the SHAs, PR/issue URLs, and FSV artifact paths in the relevant GitHub issue before moving on.
